    ...
    <Card className='flex min-w-0 flex-col gap-4 rounded-2xl p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5'>
      <div className='flex min-w-0 items-center gap-4'>
        <div className='h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-white/5'>
          {image && <img className='h-full w-full object-cover' src={image} alt={item.itemName || item.food?.name || 'Ordered food'} />}
        </div>
        <div className='min-w-0'>
          <p className='truncate font-medium text-white'>{item.itemName || item.food?.name || 'Archived menu item'}</p>
          <p className='text-sm text-gray-400'>₹{item.totalPrice}</p>
          {item.unitPrice != null && <p className='text-xs text-gray-500'>₹{item.unitPrice} each</p>}
        </div>
      </div>
